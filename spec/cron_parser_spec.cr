...
describe "CronParser#parse_element" do
  [
    ...
    it "should return #{expected} for '#{element}' when range is #{range}" do
      ...
      parser.parse_element(element, range).first.to_a.sort.should eq(expected.sort)
    end
  end
end
...
describe "CronParser#next" do
  [
    ...
    line, now, expected_next, num = data
    parser = CronParser.new(line)
    parsed_now = parse_date(now)
    expected = parse_date(expected_next)

    if num > 1
      it "returns array for '#{line}' when now is #{now}" do
        res = parser.nexts(parsed_now, num)
        res.size.should eq(num)
        res.uniq.size.should eq(num)
      end
    else
